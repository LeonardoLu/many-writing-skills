---
name: info-research
description: 围绕一个主题做主动研究：把用户的自由文本输入（一个词 / 一个句子 / 一个研究方向 / 一篇文章）通过 R-flex 管道归一化成 sub-query → 在 inbox 检索（R-α）+ fresh 搜索（R-β）→ 沉淀进 `info/research/<research-name>/` workspace（sources.md / notes.md / attachments/）。LLM 在合适时机主动建议 spawn `synthesis.md`（综合判断）/ `outline.md`（论证骨架）。本人在 chat 中说「帮我研究 X」「调研一下 Y」「围绕这个主题搜集材料」「这篇文章可以衍生出什么 sub-query」时调用。
---
...
- frontmatter 维护 `info_research_status` / `info_research_sources_count` 等字段
...
### 第 5 步：spawn 判定（synthesis / outline）

每次 skill 调用末尾，按 [`templates/synthesis.md`](templates/synthesis.md) + [`templates/outline.md`](templates/outline.md) 的 spawn 判定 prompt 评估：

- notes.md 已积累到"该收敛" → 建议 spawn `synthesis.md`
- 用户已有"先骨架再展开"的迹象 → 建议 spawn `outline.md`

仅"建议"，不主动 spawn；用户确认后才创建文件并更新 sources.md 的 `info_research_synthesis_at` / `info_research_outline_at`。
...
├── synthesis.md                # 可选 spawn；LLM 主动建议
└── outline.md                  # 可选 spawn；LLM 主动建议
```
...
业务字段一律用前缀：`info_research_*` 入 sources.md 的 frontmatter；attachments 用裸字段（不参与 dataview 聚合）。
...
- multitask：第 0 步 workspace 识别可并行 ① 列 `info/research/*` ② 读 `_taxonomy.md`
- multitask：第 1 步 R-flex K 间并行执行 K 个 sub-query（详见 `templates/r-flex.md` 第 4.3 节）
- multitask：每个 sub-query 内的 R-α inbox 检索 + R-β fresh 搜索 可并行（详见 `templates/r-flex.md` 第 6 节）
- multitask：多个 fresh URL 抓取并行
- multitask：多个 attachment 落盘并行（不同文件名、无 race）
- subagent：文章模式 + 用户希望深度衍生 → spawn explore 子代理跑 inbox 大规模相关性扫描
- subagent：sources.md H2 区块 ≥ 15 条时 spawn synthesis / outline，可用 generalPurpose 子代理跑综合，避免父 context 累积
- 串行（不要并行）：confidence 双重兜底的 forced CoT → AFCE 两个 prompt 必须时序串行；frontmatter `info_research_sources_count` 在所有并行 sub-query 返回后单点更新

## 相关文件

- 模板：`templates/r-flex.md` / `templates/sources.md` / `templates/notes.md` / `templates/attachments.md` / `templates/synthesis.md` / `templates/outline.md`
