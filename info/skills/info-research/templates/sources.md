...
info_research_result_at:                # YYYY-MM-DD；result.md spawn 时由本 skill 写入
info_research_questions_at:             # YYYY-MM-DD；question --save 首次创建 questions.md 时写入；后续 append 不更新
---
```
...
- `info_research_synthesis_at` / `info_research_outline_at` / `info_research_result_at`：仅在 spawn 对应文件时写入；用户后续手动删除 spawn 文件不回退此字段（保留历史）
- `info_research_questions_at`：仅 question 指令带 `--save` 且首次创建 `questions.md` 时写入；后续 append Q/A 区块**不**更新此字段（保留首次启用问答日志的时间）；用户后续手动删除 questions.md 不回退此字段
...
### spawn synthesis / outline / result
...
- 由 main skill 在 spawn 动作时调本模板更新 frontmatter：
  ...
  - spawn result → 写 `info_research_result_at: <today>`
- 不动其它字段
- update 模式（spawn 文件已存在 + 用户回 `update`）→ 字段不变（保留首次 spawn 日期，不覆盖）

### question --save 首次创建 questions.md

- 由 question 指令在带 `--save` 且 `questions.md` 不存在时调本模板更新 frontmatter：
  - 写 `info_research_questions_at: <today>`
- 不动其它字段
- 后续 append Q/A 区块到 `questions.md` 时**不**调本模板（不更新此字段；保留首次启用日期）
...
- **失败 1（指纹算法不一致）**：本模板列出的算法是 single source of truth；任何调用方（含 r-flex.md / synthesis.md / outline.md / result.md）都必须按本节算
