...
info_research_sources_count: 0          # 当前 H2 区块数量（写入 / 追加时同步更新）
---
```
...
- `info_research_sources_count`：每次新增 H2 区块时 +1；去重命中已有区块时不变；用户手动删 H2 时不回退（容忍漂移，不强校验）
...
- `info_research_sources_count` 写 1

### 追加新 H2 区块

- 在文件末尾追加（保持时间顺序，最新在底）
- frontmatter 的 `info_research_sources_count` += 1
...
- frontmatter `info_research_sources_count` 按实际追加数量更新
...
- 不追加新 H2 区块
- 不更新 `info_research_sources_count`
...
- **失败 2（追加时漏更 frontmatter）**：每次追加 H2 必同步 `info_research_sources_count`；不允许"只写正文不更字段"
- **失败 3（H2 区块缺字段）**：H2 区块的 5 字段（来源 / 类型 / 指纹 / 采集时刻 / 摘要）全必填；缺则补"未知"或"待补"占位，不允许整字段省略
