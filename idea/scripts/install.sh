#!/usr/bin/env bash
# Install the `idea` skill group into the target vault.
#
# Steps:
#   1. Run prepare-vault.sh to ensure the ideas/ layout exists in the vault.
#   2. Copy each skill from idea/skills/idea-* into:
#        <vault>/.<tool>/skills/<skill>/
#      for each requested tool.
#
# Usage:
#   idea/scripts/install.sh --vault <path> [--tool cursor|codex|claude|agents|all] [--dry-run]
